...
func TestToggleOffMovesGroupToMemory(t *testing.T) {
	...
	assert.Contains(t, response.Body.String(), "session only")
}

func TestToggleOnWritesBackToSettings(t *testing.T) {
	settingsPath := writeHookSettings(t, nil)
	server := newTestServer(t, &Options{SettingsPath: settingsPath})
	server.disabledHooks.Add("Stop", config.HookGroup{Hooks: []config.Hook{{Command: "say done", Type: "command"}}})
...
func TestFlushAndRestoreRoundTrip(t *testing.T) {
	...
	require.NoError(t, server.FlushDisabledHooks())
	tmpPath := disabledHooksTmpPath(settingsPath)
	_, err := os.Stat(tmpPath)
	require.NoError(t, err, "flush left no tmp file")

	restored := newTestServer(t, &Options{SettingsPath: settingsPath})
	require.Len(t, restored.disabledHooks.Snapshot()["Stop"], 1)

	_, err = os.Stat(tmpPath)
	assert.True(t, os.IsNotExist(err), "tmp file not consumed at boot")
}

func TestFlushEmptyStoreLeavesNoFile(t *testing.T) {
	settingsPath := writeHookSettings(t, nil)
	server := newTestServer(t, &Options{SettingsPath: settingsPath})

	require.NoError(t, server.FlushDisabledHooks())
	_, err := os.Stat(disabledHooksTmpPath(settingsPath))
