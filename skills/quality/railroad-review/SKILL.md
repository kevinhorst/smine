...
description: Review code changes against project conventions, specs, and definition-of-done — parallel review directions merged into one station review. Trigger on /railroad-review or "review this branch/PR/my changes". Args — lanes: lanes per direction (default 2); scope: branch|wip|all (default dynamic); refute: severity threshold; chunks: layers|groups; focus: special-focus direction; maxFiles/maxLines, base-red: gates.
...
argument-hint: "[scope] [base branch] [spec path] [focus]"
---
...
**Preconditions:** an interactive non-plan-mode session with the Workflow tool (MODES-002); the project's context docs; branch/all scopes need a diff base; a green base (PROC-009, waivable via `base-red`); the correctness direction needs a spec, plan, or request doc (**dropped** per DIR-004 without one).

**Workflow position:** standalone (see README.md § Skill map, smine repo).

## Args

- lanes: parallel lanes per direction (default 2); a directive like `lanes: opus-medium` maps onto per-lane model/effort overrides.
- scope: what to review — `branch` (committed range, fork point → head tip), `wip` (working tree only: staged, unstaged, and untracked; base = HEAD), `all` (fork point → working tree including untracked). Absent ⇒ dynamic default: `branch` when the tree is clean, `all` when dirty — the chosen scope and why is stated in the review record.
...
- maxFiles / maxLines: size cap over the **reviewed code only** — the PROC-005 pre-excluded set (vendor/, generated, deps, lockfiles) never counts. Defaults 30 files / 2000 changed lines; over either bound → PROC-008.
- refute: severity threshold for per-candidate refutation — candidates at that severity **and above** get a fresh refuter each. `blocker | major | minor | nit | info` (loglevel-style: `blocker` = blockers only, `info` = every candidate); `none` disables the stage (the station is the sole second confirmation). Default `major`.
- base-red: `expected` waives the base-health gate (PROC-009) — the review proceeds on a red base, stated in the review record.
- chunks: experimental chunked fan-out — `chunks: layers` (the dispatcher derives dependency-ordered layers from the repo's actual structure) or explicit pathspec groups, e.g. `chunks: domain=internal/model/**; api=internal/server/**`. Absent ⇒ off, lanes read the whole diff. See MODES-005.
...
**SKILL-RAILROADREVIEW-DIR-001** `[review]` — **Definitions:** direction = review lens. Lane = one agent walking one direction. Dispatcher = the session running this skill. Defaults: code-style, correctness, critical; each direction fans to `lanes` lanes.
...
**SKILL-RAILROADREVIEW-MODES-003** `[step]` — **Workflow invocation:** the dispatcher invokes the bundled workflow (`workflows/railroad-review.js`, base directory stated at skill load) once per round: `{scriptPath, args: {base, head, baseCommit, headCommit, commitsAhead, scope, riskMap, chunks, refute, artifactsDir, diffFiles, round, directions, lanes, contextRefs, focus, laneModel, laneEffort, rejectedFindings}}` — `args` as a real JSON object. `artifactsDir` = session scratchpad (absolute), artifacts under `round-{r}/` outside every worktree. `rejectedFindings` = ledger content by value — worktrees fork from committed state, cannot see an uncommitted file. The range is re-resolved every round (PROC-001..004).

**SKILL-RAILROADREVIEW-MODES-004** `[step]` — **Direction selection:** diff-proportional — a small diff may drop defaults; risk map (PROC-007) high for auth/crypto/input handling/persisted data MUST add `security`. Every drop and add named with reason in the review record.

**SKILL-RAILROADREVIEW-MODES-005** `[step]` — **Chunked fan-out** (`chunks` arg, off by default): the dispatcher partitions the reviewed files (derived layers or explicit groups), states the partition. Fan-out = directions × chunks × lanes. Lane reads only its chunk, coverage per chunk. `contracts` chunk-exempt (cross-boundary). Cross-chunk suspicions flagged in notes.
...
**SKILL-RAILROADREVIEW-ROUND-006** `[step]` — **Dedup (semantic, per direction):** a lightweight grouper agent — text-only, no worktree, no code access, low effort — groups the direction's same-defect claims across lanes; the script assembles the result deterministically: best-argued survivor, **max group severity** (so any lane's higher vote still reaches the refute threshold), absorbed ids kept as `merged_from`. In doubt the grouper keeps claims separate — a wrong merge silently drops a finding, a missed merge only costs one duplicate refutation.

**SKILL-RAILROADREVIEW-ROUND-007** `[step]` — **Refutation:** every deduped claim at or above the `refute` threshold goes to a **fresh refuter agent that did not produce it** — isolated worktree (premise per ROUND-002, branch `claude/railroad-refute-{finding-id}-r{r}-{hash6}`), pipelined per direction, second confirmation per the Probe protocol.
...
**SKILL-RAILROADREVIEW-ROUND-017** `[step]` — **DoD walk:** the station marks every `actions/reviewing.md` criterion PASS/FAIL/N/A.
...
**SKILL-RAILROADREVIEW-PROBE-002** `[step]` — **Probe anatomy:** a probe is three parts: a task-lifetime rule line in an untracked `railroad-probes-r{r}.acdsl` at the worktree root (file-scoped anchor, why = the claim); a verifier under `round-{r}/probes/` (files-list + `key=value` argv, exit 0 clean, exit 1 + `file:line: message` on hit); a merged registry under `round-{r}/probes/` (repo registry + local overlay + the probe entries, absolute argv paths, timeout ≤ 60s).

**SKILL-RAILROADREVIEW-PROBE-003** `[gate]` — **Validity first:** the probe must exit 1 on a minimal fail fixture under `round-{r}/probes/fixtures/` before the real run — a probe that cannot detect its own fixture is a failed attempt.
...
**SKILL-RAILROADREVIEW-PROBE-005** `[step]` — **Probe persistence:** probe artifacts persist under `round-{r}/probes/` and travel with the review copy — every confirmed claim stays mechanically recheckable.
...
**SKILL-RAILROADREVIEW-PROC-002** `[step]` — **Base derivation:** the invocation's base, else the tracked upstream, else the repository's mainline — state which was used; `<base>` unresolvable → retry `origin/<base>`. Unresolvable base, or an empty diff (snapshot tree identical to the base tree included) ⇒ stop and **report** (EXEC-002).
...
**SKILL-RAILROADREVIEW-PROC-008** `[gate]` — **Size cap:** counted after the PROC-005 exclusions; over `maxFiles` (30) or `maxLines` (2000, insertions+deletions) ⇒ stop and **report** with a split proposal — one chunked run (`chunks: layers`) or sequential dependency-ordered layer-runs carrying findings + ledger. Caps overridable per invocation.

**SKILL-RAILROADREVIEW-PROC-009** `[gate]` — **Base health (once per review):** build + tests at `baseCommit` in a disposable worktree (Makefile targets, else stack commands, else none — stated). Red ⇒ stop and report unless `base-red: expected` (then stated: pre-existing breakage attributes to the base). Too slow ⇒ stated skip.
...
flowchart TD
    ...
    BH -- green / base-red: expected --> RM[risk map, direction selection]
    RM --> SC{size cap?}
    SC -- over 30 files / 2000 lines --> STOP2[stop and report:<br>chunked run or layer-runs]
    SC -- within --> WF[workflow round r]
