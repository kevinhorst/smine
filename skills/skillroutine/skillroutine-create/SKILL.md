...
**Workflow position:** standalone (see `docs/skill-map.md`, smine repo); the routine route is often downstream of the skill route (skill first, wrapper second, same session) or of /smine-routines (accepted proposal in `sessions/proposals/routines.json`).
...
- Required sections in order: title, `## When to use` (Use when / Don't use when / Preconditions / Workflow position linking skill-map.md), `## Args` (required when the skill takes intake args), the body, `## Model`, `## Changelog` (newest first, `- v<version> (<date>): <text>`).
...
- `changelog.json` next to SKILL.md: JSON array of `{version, date, text}`, newest first — the server-facing mirror.
- Three version surfaces stay in sync (AGENTS.md rule): frontmatter version, `## Changelog`, `changelog.json`. Every content change bumps all three; frontmatter-only metadata edits do not bump.
...
- **Single source of truth.** The leaf dir name is the identity; frontmatter `name`, the H1, and skill-map all mirror it. Renaming a skill is a dir rename plus a reference sweep to zero (excluding history: `sessions/**` batches/archive, `plans/archived/**`, and prior changelog entries), deployed with `sync --prune` so the old deployed leaf is removed.
- **No repeated family prefix.** A family sharing a long or collision-prone prefix collapses to a single-letter prefix rather than repeating the family word: `fdesign`, `fexplore`, `fchange`, `fimpact`, `fimplement` (not `feature-design` …).
...
4. **Register** — add the skill to `docs/skill-map.md` in the smine repo (chain position or standalone list).
...
- [ ] When-to-use present with all four fields; Workflow position links skill-map.md.
- [ ] Intake args declared in a `## Args` section (`- <name>: <doc>` bullets); section absent only when the skill takes none.
- [ ] Model and Changelog sections present; changelog.json exists and matches.
...
1. **Intake** — name, prompt/slash command, schedule, budget, permission mode; check `sessions/proposals/routines.json` for an accepted proposal and its caveats (e.g. non-interactive mode, loop constraints for observation routines: pin an explicit target ID, self-terminate after N no-change checks).
...
- Scaffold, bootstrap, and kickstart run against the main checkout, never a session worktree. The `claude -p` session itself runs in the routine group's own worktree (`~/.cache/claude-routine/worktrees/<group>`, branch `routine/<group>`) created and committed by `routines/_lib/worktree.sh` — routine output never lands in the main checkout's working tree. A routine's group defaults to its own name; chain members set `ROUTINE_GROUP` to a shared group (one worktree + branch, serialized by the group lock), and different groups run concurrently — nothing ever touches a sibling group's worktree.
- Routines have no remote access: `routine_worktree_publish` commits on `routine/<group>` locally — no push, no PR, no `gh`. Reviewing and locally merging the group branch closes a run; resetting it to main discards one.
- `coverage-increaser` additionally needs a target repo: `ROUTINE_TARGET_REPO` in the plist's `EnvironmentVariables` (editable from the config server's params form), falling back to the machine-local `~/.config/claude-routine/coverage-target` (absolute path, untracked like the token file) for manual non-launchd runs. Its worktree and `routine/coverage-increaser` branch live in that target repo while results and locks stay in smine.
...
- v1.2 (2026-07-31): packaging contract label rename to com.smine.routine.*
- v1.1 (2026-07-30): Command surface retired in favor of allowed-tools frontmatter (permission-manifest MVP); Model contract and format gate updated
- v1.0 (2026-07-27): merges couchskill-create v1.7 and couchroutine-create v1.5 into one arg-routed skill (skill|routine routes); effort token normalized (large → high); eval handoff → /skillroutine-eval; naming schema documents the smine pipeline prefix
