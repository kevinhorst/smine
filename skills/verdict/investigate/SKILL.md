...
- `artifactsDir`: absolute path where investigator artifacts are written, outside every worktree; optional — defaults to `<session scratchpad>/investigation` (the scratchpad directory announced in the system prompt); the skill creates the directory before the Workflow call (this skill is read-only toward the repo).
...
- Call the Workflow tool: `{scriptPath: '<this skill's base directory>/workflows/investigate.js', args: {question, inputs, artifactsDir, priorArtifact, investigators}}` — the base directory is stated when this skill loads; args is a real JSON object, never a string. `question` and `inputs` are required strings; `artifactsDir` is an absolute path outside every worktree — when the caller gives none, resolve the session scratchpad directory, `mkdir -p <scratchpad>/investigation`, and pass that path (the workflow still rejects a missing or non-absolute value so no artifact lands in the repo); `priorArtifact` and `investigators` are optional.
