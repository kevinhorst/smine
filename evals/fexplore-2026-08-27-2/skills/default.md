...
Static constraints are not restated here: `AGENTS.md`, the style guides under `context/style/` (incl. `plan.md`), and the doctrine entries under `context/rules/` govern every artifact. Missing context docs: state it once, continue with built-in baselines — never stall.
